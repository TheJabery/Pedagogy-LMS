...
class StudentController extends Controller
{
    ...
    public function index()
    {
        ...
        $students = Student::with('gender','grade','classroom','section')->whereIn('section_id', $ids)->get();
    ...
    public function attendanceReport()
    {
        ...
        $ids = DB::table('teacher_section')->where('teacher_id', auth()->user()->id)->pluck('section_id');
        $students = Student::with('classroom')->whereIn('section_id', $ids)->get();
    ...
    public function attendanceSearch(Request $request)
    {
        ...
        if ($request->student_id == 0) {
            ...
            $Students = Attendance::with('students.classroom','grade','section')->whereBetween('attendence_date', [$request->from, $request->to])->get();
            return view('pages.Teachers.dashboard.students.attendance_report', compact('Students', 'students'));
        } else {

            $Students = Attendance::with('students.classroom','grade','section')->whereBetween('attendence_date', [$request->from, $request->to])
